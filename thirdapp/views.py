...
def my_view(request):
    context = {'name': 'Sega'}
    return render(request, 'thirdapp/my_template.html', context)


class TemplIf(TemplateView):
    template_name = 'thirdapp/templ_if.html'
...
def view_for(request):
    ...
    return render(request, 'thirdapp/templ_for.html', context)
...
def author_posts(request, author_id):
    ...
    return render(request, 'thirdapp/author_posts.html', {'author': author, 'posts': posts})


def post_full(request, post_id):
    post = get_object_or_404(Post, pk=post_id)
    return render(request, 'thirdapp/post_full.html', {'post': post})
